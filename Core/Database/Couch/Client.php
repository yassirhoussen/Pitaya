...
class Client {
	...
	*/
	public function parseResponse(&$rawData) {
		if ( !strlen($rawData) ) 
			throw new DatabaseException("no data to parse");
	...
	*/
	private function _curlQuery ( $method, $url, $parameters = array(), $data = NULL) {
		if ( !in_array($method, $this->HTTP_METHODS )    )
			throw new DatabaseException("Bad HTTP method: $method");
		$url = $this->dns.$url;
	...
	*/
	public function _curlStoreFile ( $url, &$file) {
		if ( !strlen($url) )
			throw new DatabaseException("Attachment URL can't be empty");
		if ( !strlen($file) || !is_file($file) || !is_readable($file) )	
			throw new DatabaseException("Attachment file does not exist or is not readable");
